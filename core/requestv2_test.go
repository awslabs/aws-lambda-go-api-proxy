...
var _ = Describe("RequestAccessorV2 tests", func() {
	...
	Context("Retrieves API Gateway context", func() {
		...
		It("Supports cookies", func() {
			basicRequest := getProxyRequestV2("orders", "GET")
			cookies := map[string]string{
				"MyTestCookie":      "mytestvalue",
				"MyOtherTestCookie": "myothertestvalue",
			}

			for cookie, value := range cookies {
				basicRequest.Cookies = append(basicRequest.Cookies, cookie+"="+value)
			}
